...
- Consulter immédiatement en cas de douleur importante, baisse de vision, photophobie, mouches lumineuses ou sécrétions abondantes.`
  },
  {
    ...
    subitems: [
      {
        subtitle: "Chirurgie de la cornée",
        content: `
...
- Signaler immédiatement rougeur, douleur intense, baisse visuelle ou photophobie.`
      ...
      {
        subtitle: "Chirurgie de la rétine",
        content: `
    ...
    <div className="p-6 max-w-xl mx-auto space-y-4 text-center">
      ...
      <div className="space-y-4">
        {pathologies.map((item, index) => (
          <Card
            key={index}
            className={`text-left transition-opacity duration-300 ${openItem && openItem !== item.title ? "opacity-50" : "opacity-100"}`}
          >
                    ...
                    {item.subitems ? (
                      <div className="space-y-3">
                        {item.subitems.map((sub, idx) => (
                          <Card key={idx}>
                            <Accordion type="single" collapsible value={openSubitem} onValueChange={setOpenSubitem}>
                              <AccordionItem value={sub.subtitle}>
                                <button className="w-full flex justify-between items-center p-3" onClick={() => setOpenSubitem(openSubitem === sub.subtitle ? null : sub.subtitle)}>
                                  <span className="font-medium">{sub.subtitle}</span>
                                  <ChevronDown className={`transition-transform ${openSubitem === sub.subtitle ? "rotate-180" : "rotate-0"}`} />
                                </button>
                                {openSubitem === sub.subtitle && (
                                  <CardContent className="p-4 whitespace-pre-wrap text-sm text-gray-700">
                                    {sub.content}
                                  </CardContent>
                                )}
