...
- Consulter immédiatement en cas de douleur importante, baisse de vision, photophobie, mouches lumineuses ou sécrétions abondantes.\`
  },
  {
    ...
    subItems: [
      {
        title: "Post-opératoire chirurgie de la cornée",
        content: \`
...
- Signaler immédiatement rougeur, douleur intense, baisse visuelle ou photophobie.\`
      ...
      {
        title: "Post-opératoire chirurgie de la rétine",
        content: \`
...
- Surveiller l’apparition de points noirs, douleurs, ou baisse de vision rapide.\`
      ...
      {
        title: "Post-opératoire chirurgie palpébrale",
        content: \`
        ...
        {pathologies.map((item, index) => (
          <Card
            ...
            className={`text-left transition-opacity duration-300 ${
              openItem === null || openItem === item.title ? "opacity-100" : "opacity-50"
            }`}
          >
                    ...
                    {item.subItems ? (
                      <div className="space-y-2">
                        {item.subItems.map((sub, subIndex) => (
                          <div key={subIndex} className="border rounded">
                            <button
                              className="w-full text-left p-2 font-semibold bg-gray-100"
                              onClick={() => setOpenSubItem(openSubItem === sub.title ? null : sub.title)}
                            >
                              {sub.title}
                            </button>
                            {openSubItem === sub.title && (
                              <div className="p-2 text-sm text-gray-700 whitespace-pre-wrap">{sub.content}</div>
                            )}
                          </div>
                        ))}
                      </div>
